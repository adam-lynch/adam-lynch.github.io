...
; (function () {
  ...
  if (s && s.dataset.goatcounterSettings) {
    try { var set = JSON.parse(s.dataset.goatcounterSettings) } catch (err) { console.error('invalid JSON in data-goatcounter-settings: ' + err) }
    for (var k in set) {
      if (['no_onload', 'no_events', 'allow_local', 'allow_frame', 'path', 'title', 'referrer', 'event'].indexOf(k) > -1) { window.goatcounter[k] = set[k] }
    }
  }
  ...
  var urlencode = function (obj) {
    var p = []
    for (var k in obj) {
      if (obj[k] !== '' && obj[k] !== null && obj[k] !== undefined && obj[k] !== false) { p.push(enc(k) + '=' + enc(obj[k])) }
    }
    return '?' + p.join('&')
  }
  ...
  var on_load = function (f) {
    if (document.body === null) { document.addEventListener('DOMContentLoaded', function () { f() }, false) } else { f() }
  }
  ...
  window.goatcounter.count = function (vars) {
    ...
    if (navigator.sendBeacon) { navigator.sendBeacon(url) } else { // Fallback for (very) old browsers.
  ...
  window.goatcounter.get_query = function (name) {
    ...
    for (var i = 0; i < s.length; i++) {
      if (s[i].toLowerCase().indexOf(name.toLowerCase() + '=') === 0) { return s[i].substr(name.length + 1) }
    }
  }
