<template>
  <article class="post">
    <h3 class="post-title-wrapper">
      <nuxt-link :to="to" class="post-title js_track-link-clicks">{{ article.title }}</nuxt-link>
    </h3>

    <div class="post-meta">
        <strong class="post-blog-title" v-if="original">
            <a :href="article.original.blogLink" class="post-blog-link js_track-link-clicks">{{ article.original.blogName }}</a>
        </strong>
        <time class="post-blog-date">{{ prettyDate }}</time>
    </div>

    <div class="post-content" v-html="article.rendered.replace(/<h1[^>]*>([^<]*)<\/h1>/i, '')"></div>
  </article>
